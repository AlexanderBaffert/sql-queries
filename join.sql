...
SELECT genres.id, genres.name, AVG(milliseconds) AS avg_length
FROM genres
JOIN tracks
  ON (tracks.genre_id = genres.id)
GROUP by genres.id
...
SELECt albums.id AS album_id, albums.title, SUM(milliseconds) AS total_length
FROM albums
JOIN tracks
...
SELECT albums.id AS album_id, albums.title, AVG(milliseconds) AS avg_length
FROM albums
JOIN tracks
  ON (tracks.album_id = albums.id)
GROUP By albums.id
...
SELECT albums.id AS albums_id, albums.title, SUM(unit_price) AS total_price
FROM albums
JOIN tracks
  ON (tracks.album_id = albums.id)
GROUP BY albums.id
...
SELECT tracks.id AS track_id, tracks.name, SUM(quantity) AS total_sold
FROM tracks
JOIN invoice_lines
